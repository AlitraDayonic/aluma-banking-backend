...
const { authenticate: auth5 } = require('../middleware/auth');
const { validate: validate5, schemas: schemas5 } = require('../middleware/validate');

router5.post('/deposits', auth5, validate5(schemas5.deposit), fundingController.initiateDeposit);
router5.post('/withdrawals', auth5, validate5(schemas5.withdrawal), fundingController.requestWithdrawal);
router5.post('/transfers', auth5, fundingController.internalTransfer);
router5.post('/transfers/external', auth5, fundingController.externalTransfer);
